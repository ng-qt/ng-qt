...
function tsResolve(
  ...
  try {
    ...
    const compilerOptions: ts.CompilerOptions =
      tsConfig.options || ts.getDefaultCompilerOptions();
    ...
    if (
      ...
      resolutionResult.resolvedModule.resolvedFileName
    ) {
      result = relative(
        dirname(containingFilePath),
        resolutionResult.resolvedModule.resolvedFileName,
      );
    }
  } catch (err) {}
...
export function findBootstrapModuleCallInSource(
  source: ts.SourceFile,
): ts.CallExpression | null {
  return (
    findMethodCallInSource(source, 'bootstrapModule') ||
    findMethodCallInSource(source, 'bootstrapModuleFactory')
  );
}

export function findNativeScriptPlatformCallInSource(
  source: ts.SourceFile,
): ts.CallExpression | null {
  return (
    findMethodCallInSource(source, 'platformNativeScriptDynamic') ||
    findMethodCallInSource(source, 'platformNativeScript')
  );
}

export function findMethodCallInSource(
  source: ts.SourceFile,
  methodName: string,
): ts.CallExpression | null {
  const allMethodCalls = collectDeepNodes<ts.CallExpression>(
    source,
...
export function findBootstrappedModulePathInSource(
  source: ts.SourceFile,
): string {
  ...
  const appModulePath = getExpressionImportPath(
    source,
    bootstrapCall.arguments[0],
  );

  return appModulePath;
}

export function findNativeScriptPlatformPathInSource(
  source: ts.SourceFile,
): string {
  ...
  const nsPlatformImportPath = getExpressionImportPath(
    source,
    nsPlatformCall.expression,
  );
...
export function getObjectPropertyMatches(
...
export function getDecoratorMetadata(
  ...
  const angularImports: { [name: string]: string } = collectDeepNodes(
    source,
...
export function angularImportsFromNode(
...
function getExpressionImportPath(
  source: ts.SourceFile,
