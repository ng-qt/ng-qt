...
function normalizeBuildOptions(options, root, sourceRoot) {
  return {
    ...
    webpackConfig: options.webpackConfig ? path_1.resolve(root, options.webpackConfig) : options.webpackConfig,
  };
}
...
function normalizeAssets(assets, root, sourceRoot) {
  return assets.map(asset => {
    if (typeof asset === 'string') {
      ...
      const input = isDirectory ? resolvedAssetPath : path_1.dirname(resolvedAssetPath);
